    ...
    <div class="das-panel__header border-bottom py-3 px-4 d-flex align-items-center justify-content-between flex-wrap gap-3"
      ...
      <div class="d-flex align-items-center gap-3">
        ...
        <span class="das-chip --info d-none d-sm-inline-flex">
          {{ method_exists($absensi, 'total') ? $absensi->total() : count($absensi) }} Absensi
        </span>
      </div>
    </div>
    <div class="das-panel__body p-0">
      <div id="absensiGuruTableContainer">
        @include('admin.absensi-guru.table')
      </div>
    </div>
  </div>
@endsection
...
@section('page-script')
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      const container = document.getElementById('absensiGuruTableContainer');
      ...
      function fetchData(page = 1) {
        ...
        const url = `{{ route('admin.absensi-guru.index') }}?page=${page}&search=${search}&per_page=${perPage}&status=${status}&tanggal=${tanggal}`;
