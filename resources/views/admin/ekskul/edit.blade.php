...
@section('content')
        ...
        <div class="card-body p-4">
          <div class="d-flex align-items-center gap-3">
            <div class="rounded d-flex align-items-center justify-content-center shadow-sm"
              ...
              <i class="ti tabler-edit text-warning fs-3"></i>
            </div>
            <div>
  ...
  {{-- ═══════════ FORM ═══════════ --}}
  <form action="{{ route('admin.ekskul.update', $ekskul->id) }}" method="POST" id="ekskulForm">
    @csrf
    @method('PUT')
        ...
        <div class="row g-4">
          ...
          <div class="col-md-8">
            ...
            <input type="text" name="nama" id="nama"
              ...
              value="{{ old('nama', $ekskul->nama) }}" required maxlength="255">
          ...
          <div class="col-md-4">
            ...
            <select name="kategori" id="kategori"
              ...
              @foreach(['wajib'=>'Wajib','pilihan'=>'Pilihan','olahraga'=>'Olahraga','seni'=>'Seni','akademik'=>'Akademik','lainnya'=>'Lainnya'] as $val=>$label)
                <option value="{{ $val }}" {{ old('kategori', $ekskul->kategori) == $val ? 'selected' : '' }}>{{ $label }}</option>
              @endforeach
            </select>
          ...
          <div class="col-12">
            ...
            <textarea name="deskripsi" id="deskripsi" rows="3"
              ...
              placeholder="Deskripsikan kegiatan ekskul ini...">{{ old('deskripsi', $ekskul->deskripsi) }}</textarea>
          ...
          <div class="col-md-4">
            ...
            <input type="number" name="kuota" id="kuota"
              ...
              value="{{ old('kuota', $ekskul->kuota) }}" min="1">
          ...
          <div class="col-md-4">
            ...
            <div class="form-check form-switch mt-2">
              ...
              <input class="form-check-input" type="checkbox" role="switch" name="status" id="status" value="1"
                {{ old('status', $ekskul->status) == '1' ? 'checked' : '' }}
          ...
          <div class="col-md-4">
            ...
            <select name="icon" id="icon"
              ...
              @foreach([
                ...
                <option value="{{ $val }}" {{ old('icon', $ekskul->icon) == $val ? 'selected' : '' }}>{{ $label }}</option>
              @endforeach
            </select>
    ...
    {{-- ═══════════ SECTION: JADWAL ═══════════ --}}
    <div class="section-card mb-4 slide-in-up" x-data="jadwalManager({{ json_encode($ekskul->jadwal->map(function($j) { return ['hari' => $j->hari, 'jam_mulai' => $j->jam_mulai, 'jam_selesai' => $j->jam_selesai, 'lokasi' => $j->lokasi]; })->toArray()) }})">
    ...
    {{-- ═══════════ SECTION: PEMBINA ═══════════ --}}
    <div class="section-card mb-4 slide-in-up" x-data="pembinaManager({{ json_encode($ekskul->pembina->map(function($p) { return ['guru_id' => (string)$p->guru_id, 'jabatan' => $p->jabatan]; })->toArray()) }})">
      <div class="section-card__head">
        <div class="section-card__title">
    ...
    <div class="d-flex align-items-center justify-content-end gap-3 slide-in-up">
      ...
      <button type="submit" class="btn btn-warning fw-semibold px-4 shadow-sm text-white">
        <i class="ti tabler-device-floppy me-1"></i> Perbarui Ekskul
      </button>
    </div>
  </form>

  {{-- Info Ringkasan --}}
  <div class="card border-0 shadow-sm mt-4 slide-in-up"
    style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08) !important;">
    <div class="card-body p-4">
      <div class="row g-3">
        <div class="col-md-6">
          <div class="d-flex align-items-center gap-3">
            <div class="rounded d-flex align-items-center justify-content-center"
              style="width:40px;height:40px;background:rgba(0,207,232,.15);border:1px solid rgba(0,207,232,.3);border-radius:8px !important;">
              <i class="ti tabler-users text-info"></i>
            </div>
            <div>
              <div class="text-white-50 small">Anggota Terdaftar</div>
              <strong class="text-white">{{ $ekskul->anggota_count }}</strong>
            </div>
          </div>
        </div>
        <div class="col-md-6 text-md-end">
          <a href="{{ route('admin.ekskul.anggota.index', $ekskul->id) }}" class="btn btn-label-info btn-sm">
            <i class="ti tabler-users me-1"></i> Kelola Anggota
          </a>
          <a href="{{ route('admin.ekskul.absensi.index', $ekskul->id) }}" class="btn btn-label-success btn-sm ms-2">
...
@section('page-script')
<script>
  function jadwalManager(initialRows) {
    return {
      rows: initialRows || [],
      addRow() {
  ...
  function pembinaManager(initialRows) {
    return {
      rows: initialRows || [],
      addRow() {
