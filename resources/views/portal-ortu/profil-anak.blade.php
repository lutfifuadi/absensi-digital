...
{{-- ═══════════════════════════════════════════════════════
     SECTION 1: HERO HEADER — Identitas Anak + Live Clock
═══════════════════════════════════════════════════════ --}}
  ...
  <div class="das-hero__inner flex-column flex-md-row align-items-start align-items-md-center justify-content-between gap-4">
    ...
    <div class="das-hero__identity">
      <div class="das-hero__logo-wrapper">
        @php
          $avatarUrl = 'https://ui-avatars.com/api/?name=' . urlencode($anak->nama_lengkap) . '&size=120&background=7367f0&color=fff';
          if ($anak->foto) {
              if (strlen($anak->foto) > 30) {
                  $avatarUrl = 'https://drive.google.com/thumbnail?id=' . $anak->foto . '&sz=w200&_t=' . time();
              } else {
                  $avatarUrl = asset('storage/' . $anak->foto);
              }
          }
        @endphp
        <img class="das-hero__logo" src="{{ $avatarUrl }}" alt="Foto {{ $anak->nama_lengkap }}" style="object-fit: cover;">
      ...
      <div class="das-hero__meta">
        <div class="das-hero__badge">
          ...
          NIS / NISN: {{ $anak->nis ?: $anak->nisn }}
        </div>
        <h3 class="das-hero__school text-gradient-gold mb-1" style="font-size: 1.35rem; line-height: 1.3;">{{ $anak->nama_lengkap }}</h3>
        <p class="das-hero__welcome mb-0">
          <i class="ti tabler-school me-1"></i>{{ $anak->kelas->nama ?? 'Tanpa Kelas' }} 
          <span class="opacity-50 mx-1">•</span> 
          <i class="ti tabler-calendar-event me-1"></i>TA {{ $anak->tahunAkademik->nama ?? '-' }}
        </p>
      </div>
    </div>

    {{-- Quick Hero Actions untuk Orang Tua --}}
    <div class="das-hero__actions d-flex gap-2 flex-wrap ms-md-auto">
      <a href="{{ route('ortu.anak.absensi', $anak->id) }}" class="btn btn-primary btn-sm d-flex align-items-center gap-1 shadow-sm">
        <i class="ti tabler-calendar-stats"></i> Lihat Absensi
      </a>
      <a href="{{ route('ortu.izin-sakit.create', ['siswa_id' => $anak->id]) }}" class="btn btn-warning text-white btn-sm d-flex align-items-center gap-1 shadow-sm">
        <i class="ti tabler-file-plus"></i> Ajukan Izin
      </a>
    </div>
  </div>
</div>{{-- /das-hero --}}
      ...
      <div class="card-header pb-2 d-flex align-items-center justify-content-between">
        <div class="d-flex align-items-center gap-2">
          ...
          <div>
            ...
            <small class="text-body-secondary">Detail data diri anak</small>
          </div>
        </div>
      ...
      <div class="card-body pt-2">
        <ul class="list-group list-group-flush border-top border-bottom my-3">
          <li class="list-group-item d-flex justify-content-between align-items-center px-0 py-2 border-0 border-bottom">
            ...
            <span class="fw-bold font-monospace">{{ $anak->nisn }}</span>
          </li>
          <li class="list-group-item d-flex justify-content-between align-items-center px-0 py-2 border-0 border-bottom">
            <span class="text-body-secondary small"><i class="ti tabler-badge me-1"></i> NIS</span>
            <span class="fw-bold font-monospace">{{ $anak->nis ?: '-' }}</span>
          </li>
          <li class="list-group-item d-flex justify-content-between align-items-center px-0 py-2 border-0 border-bottom">
            <span class="text-body-secondary small"><i class="ti tabler-gender-male-female me-1"></i> Jenis Kelamin</span>
            <span class="fw-semibold">{{ $anak->jenis_kelamin == 'L' ? 'Laki-laki' : 'Perempuan' }}</span>
          </li>
          <li class="list-group-item d-flex justify-content-between align-items-center px-0 py-2 border-0 border-bottom">
            <span class="text-body-secondary small"><i class="ti tabler-map-pin me-1"></i> Tempat, Tgl Lahir</span>
            <span class="fw-semibold text-end">{{ $anak->tempat_lahir ?? '-' }}, {{ $anak->tanggal_lahir ? \Carbon\Carbon::parse($anak->tanggal_lahir)->translatedFormat('d M Y') : '-' }}</span>
          </li>
          <li class="list-group-item d-flex justify-content-between align-items-center px-0 py-2 border-0 border-bottom">
            <span class="text-body-secondary small"><i class="ti tabler-chalkboard-teacher me-1"></i> Wali Kelas</span>
            <span class="fw-semibold text-primary">{{ $anak->kelas->waliKelas->nama_lengkap ?? $anak->kelas->waliKelas->nama ?? '-' }}</span>
          </li>
          <li class="list-group-item d-flex justify-content-between align-items-center px-0 py-2 border-0">
            ...
            <span class="fw-semibold font-monospace">{{ $anak->no_hp_ortu }}</span>
          </li>
        </ul>
          ...
          <div class="d-flex flex-column gap-3 w-100">
            ...
            <div class="w-100 p-3 bg-white rounded shadow-sm text-center border">
              <div class="barcode-svg-container w-100 mb-2" style="height: 60px;">
                {!! App\Support\BarcodeGenerator::renderSvg($anak->nis ?: $anak->nisn ?: 'SISWA'.$anak->id) !!}
              </div>
              <div class="text-muted font-monospace" style="font-size: 0.75rem;">Barcode: {{ App\Support\BarcodeGenerator::getFormattedData($anak->nis ?: $anak->nisn ?: 'SISWA'.$anak->id) }}</div>
            </div>
          </div>
        </div>
      </div>
