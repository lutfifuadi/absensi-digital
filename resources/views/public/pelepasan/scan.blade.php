...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .toast-icon { width: 50px; height: 50px; border-radius: 5px; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; flex-shrink: 0; }
...
<nav class="nav">
  ...
  <div class="nav-right" style="display:flex; align-items:center; gap:1.5rem;">
    <div id="nav-date" style="font-size:0.8rem; font-weight:700; color:#94a3b8;"></div>
    <form action="{{ route('public.pelepasan.logout') }}" method="POST" style="margin:0;">
      @csrf
...
<div class="layout">
  ...
  <div class="camera-panel">
    ...
    <div class="idle-screen" id="idle-screen">
      ...
      <div class="error-box" id="error-box" style="display:none; background:rgba(234,84,85,0.1); border:1px solid var(--das-danger); color:var(--das-danger); padding:1rem; border-radius:5px; font-size:0.85rem; max-width:300px; text-align:center;"></div>
  ...
  <div class="sidebar">
    ...
    <div class="stats-row">
      <div class="stat-item">
        <div class="stat-num" id="stat-hadir" style="color:var(--das-success);">{{ $totalHadir }}</div>
...
<script>
  ...
  const SCAN_URL  = "{{ route('public.pelepasan.process') }}";
  const LOGIN_URL = "{{ route('public.pelepasan.index') }}";
  const DISMISS   = 3000;
  const DEBOUNCE  = 3500;
  ...
  // ── Counters ──
  let cntHadir = {{ $totalHadir }}, cntFail = 0;
  function incrStat(type) {
    if (type === 'success') {
      cntHadir++;
      document.getElementById('stat-hadir').textContent = cntHadir;
    } else {
      cntFail++;
      document.getElementById('stat-fail').textContent = cntFail;
    }
  }
