    ...
    private function triggerLazyCron(): void
    {
        try {
            ...
            if (!$lastRun || now()->diffInMinutes($lastRun) >= 10) {
                ...
                // Run schedule silently in the background using Symfony Process
                // Using array args avoids shell interpretation (command injection safe)
                $process = new \Symfony\Component\Process\Process(
                    [PHP_BINARY, base_path('artisan'), 'schedule:run']
                );
